...
while(True):
    ...
    ret, mask = cv2.threshold(frame,50,255,cv2.THRESH_BINARY)
    mask = cv2.GaussianBlur(mask,(9,9),0)
    mask = cv2.Canny(mask, 100, 200)

    cnts = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) #not cunts, contours!
    cnts = imutils.grab_contours(cnts)

    coordlist = []
    def gimmeLocations(cnts):
